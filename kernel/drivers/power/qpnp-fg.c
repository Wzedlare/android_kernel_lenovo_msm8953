				  ...
				  const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENABLE_JEITA_DETECTION:
		...
		if (chip->use_soft_jeita_irq) {
			enable_irq(chip->batt_irq[JEITA_SOFT_COLD].irq);
			enable_irq_wake(chip->batt_irq[JEITA_SOFT_COLD].irq);
			enable_irq(chip->batt_irq[JEITA_SOFT_HOT].irq);
			enable_irq_wake(chip->batt_irq[JEITA_SOFT_HOT].irq);
		} else {
			disable_irq_wake(chip->batt_irq[JEITA_SOFT_COLD].irq);
			disable_irq_nosync(chip->batt_irq[JEITA_SOFT_COLD].irq);
			disable_irq_wake(chip->batt_irq[JEITA_SOFT_HOT].irq);
			disable_irq_nosync(chip->batt_irq[JEITA_SOFT_HOT].irq);
		}
		break;
...
static irqreturn_t fg_jeita_soft_hot_irq_handler(int irq, void *_chip)
{
	...
	if ( !(is_charger_available(chip))) {
		pr_err("battery psy is not ready, return\n");
		return false;
	}
