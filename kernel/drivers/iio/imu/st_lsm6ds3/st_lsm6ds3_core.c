				...
				short new_fifo_decimator[ST_INDIO_DEV_NUM + 1])
{
	...
	printk("%s-0 trigger-odr:%d v_odr:%d-%d-%d hw_odr:%d-%d-%d\n", __func__, trigger_odr,
		new_v_odr[ST_MASK_ID_ACCEL], new_v_odr[ST_MASK_ID_GYRO], new_v_odr[ST_MASK_ID_EXT0],
		new_hw_odr[ST_MASK_ID_ACCEL], new_hw_odr[ST_MASK_ID_GYRO], new_hw_odr[ST_MASK_ID_EXT0]);
	...
	printk("%s-1 decimators:%d-%d-%d\n", __func__, accel_decimator, gyro_decimator, ext_decimator);
						...
						unsigned int odr, bool force)
{
	...
	printk("%s-0-%d %d %d\n", __func__, sdata->sindex, odr, force);
	...
	if (scan_odr) {
		...
		if (!force) {
			if ((sdata->cdata->sensors_enabled & BIT(sdata->sindex)) == 0) {
				printk("%s-1-%d sensor is off, changing odr variable only to %dHz\n", __func__, sdata->sindex, st_lsm6ds3_odr_table.odr_avl[i].hz);
		...
		if (sdata->cdata->hw_odr[sdata->sindex] == st_lsm6ds3_odr_table.odr_avl[i].hz) {
			printk("%s-2-%d sensor do not modify odr\n", __func__, sdata->sindex);
			reg_value = 0xff;
		} else {
			printk("%s-3-%d sensor not modify odr\n", __func__, sdata->sindex);
	...
	if (sdata->cdata->sensors_use_fifo > 0) {
		...
		printk("%s-4-%d check v_odr & hw_odr %d %d - %d %d - %d %d\n", __func__, sdata->sindex,
			temp_v_odr[ST_MASK_ID_ACCEL], temp_hw_odr[ST_MASK_ID_ACCEL],
			temp_v_odr[ST_MASK_ID_GYRO], temp_hw_odr[ST_MASK_ID_GYRO],
			temp_v_odr[ST_MASK_ID_EXT0], temp_hw_odr[ST_MASK_ID_EXT0]);
		...
		if (fifo_conf_changed) {
			...
			printk("%s-5-%d sip : %d %d %d\n", __func__, sdata->sindex, samples_in_pattern[0], samples_in_pattern[1], samples_in_pattern[2]);
		...
		printk("%s-6-%d check v_odr %d\n", __func__, sdata->sindex, sdata->cdata->v_odr[sdata->sindex]);
	} else {
				...
				struct device_attribute *attr, char *buf)
{
	...
	printk("%s odr = %d\n", __func__, sdata->cdata->v_odr[sdata->sindex]);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sdata->cdata->v_odr[sdata->sindex] != odr) {
		if ((sdata->sindex == ST_MASK_ID_ACCEL) && (sdata->cdata->sensors_enabled & BIT(ST_MASK_ID_ACCEL))) {
			printk("change odr prima %d\n", sdata->sindex);
			err = lsm6ds3_enable_accel(sdata->cdata, ST_MASK_ID_ACCEL, odr);
		} else {
			printk("change odr seconda %d\n", sdata->sindex);
			err = st_lsm6ds3_set_odr(sdata, odr, false);
		}
	}
	printk("%s lsm6ds3 set odr %d, odr_previous %d ,err %d\n",
		__func__, odr, sdata->cdata->v_odr[sdata->sindex], err);
#endif /* CONFIG_ST_LSM6DS3_XL_DATA_INJECTION */
	mutex_unlock(&sdata->cdata->odr_lock);
